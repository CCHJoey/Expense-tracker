    ...
    <main className="flex min-h-screen flex-col items-center justify-between sm:p-24 p-24">
      <div className="z-10 max-w-5xl w-full items-center justify-between font-mono text-sm">
        ...
        <div className="bg-slate-800 p-4 rounded-lg">
          <form className="grid grid-cols-6 tiems-center text-black">
          ...
          <ul>
            {items.map((item, id) => (
              <li key={id} className="my-4 w-full flex justify-between bg-slate-950">
                ...
                <button onClick={() => deleteItem(item.id)} className="ml-8 p-4 border-1-2 border-slate-900 hover:bg-slate-900 w-16 text-white">X</button>
              </li>
            ))}
          </ul>
          {items.length < 1 ? ('') : (
